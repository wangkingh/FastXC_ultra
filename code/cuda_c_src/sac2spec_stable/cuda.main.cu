...
int main(int argc, char **argv)
{
    ...
    /* ---------- 计算滑动步长 ---------- */
    int shift_length_pts = static_cast<int>(argument.segshift / delta + 0.5f);
    if (shift_length_pts < 1)            // segshift 太小或忘填 → 退化为“无重叠”
        shift_length_pts = nseg;

    /* ---------- 计算可滑动次数 ---------- */
    int nstep = 0;
    if (nseg <= npts) {
        nstep = (npts - nseg) / shift_length_pts + 1;
        if (nstep < 1) nstep = 1;        // 极短记录保护
    } else {
        nstep = 1;                       // 窗口比记录长，整条记录视为一个片段
    }
    printf("nstep = %d\n", nstep);
    ...
    for (size_t finish_batch = 0; finish_batch < nValid_batch; finish_batch += batch)
    {
        ...
        for (int stepidx = 0, done_step = 0; stepidx < nstep; stepidx++)
        {
            ...
            // copy sacdata to GPU
            cudaMemcpy2D(d_sacdata, nseg * sizeof(float),
                        h_sacdata + stepidx * shift_length_pts,   // ← 新的窗口起点
                        npts * sizeof(float),
                        nseg * sizeof(float), proc_cnt,
