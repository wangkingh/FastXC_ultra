...
def rotate_deployer(output_dir: str, cpu_count: int, log_file_path: str, dry_run: bool):
    ...
    for cmd_file_name in sorted(cmd_files):
        ...
        dep_logger.info(
            f"[rotate_cmd_deployer] Found {len(label_cmds)} commands for label '{label}' in {file_path}"
        )
        if not label_cmds:
            continue
        executor = MultiDeviceTaskExecutor.from_threadpool(
            ...
            task_description="[Rotating]",
            queue_size=1,
            max_retry=3,
            enable_progress_bar=True,
        )
        executor.set_command_list(label_cmds)
        dep_logger.info(f"Starting rotation for label: {label}\n")
